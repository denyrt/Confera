...
and the global handler handles unexpected failures. Domain Try methods return
typed input failures; Application maps them to its feature errors. Throwing
Domain wrappers share the rules but are not used for expected request rejection.
Known persistence exceptions are adapted at the Application boundary. Infrastructure
...
[RF1 boundary design](docs/plans/readability-refactoring-plan.md) and
[RF2 Domain validation](docs/plans/domain-validation-results-plan.md).
